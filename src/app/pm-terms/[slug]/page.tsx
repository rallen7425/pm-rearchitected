...
export async function generateStaticParams() {
  const slugs = await listAllTermSlugs("pm");
...
export async function generateMetadata({
  ...
  const term = await getTermBySlug(slug, "pm");
  if (!term) return { title: "Term Not Found — Product Management, Re-Architected" };
  return {
    title: `${term.canonical_term} — Product Management Terms — Product Management, Re-Architected`,
    description: term.short_definition,
  };
}

export default async function PmTermPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [term, categories] = await Promise.all([getTermBySlug(slug, "pm"), listCategories("pm")]);
  if (!term) notFound();
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        <div className="container pt-8 pb-20 max-w-3xl">
          <Link href="/pm-terms" className="text-sm text-primary hover:underline">
